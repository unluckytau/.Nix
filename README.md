...
cp /etc/nixos/hardware-configuration.nix /home/$USER/.Nix/system/hardware-configuration.nix
```
...
cp /home/$USER/.Nix/etc/display.lua /home/$USER/.local/state/hypr/
cp /home/$USER/.Nix/etc/misc.lua /home/$USER/.local/state/hypr/
```

> #### Additional Installation Notes.
> 
> - If on a different device, disable import for `./wdblue.nix` in `.Nix/system/default.nix`. Delete `./wdblue.nix` module afterwards.
> - If on a non-nvidia device, disable import for `./nvidia.nix` in `.Nix/system/default.nix`. Delete `./nvidia.nix` module afterwards.

5. Disable import for `./nixvim` in `.Nix/home/default.nix`.
7. Disable import for `./misc.nix` in `.Nix/system/default.nix`.
